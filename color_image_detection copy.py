...
def draw_circle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing:
            ...
            # Only add the circle if it has a valid radius
...
def select_colors(image):
    ...
    while True:
        ...
        if choice.isdigit() and int(choice) in range(len(predefined_colors)):
            ...
            cv2.setMouseCallback('Calibrate Color', draw_circle)
...
def main():
    ...
    while True:
        ...
        if not show_results:
            ...
            cv2.putText(display_image, "Press 'Enter' to analyze, 'c' to clear all circles, 'q' to quit", 
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        else:
            cv2.putText(display_image, "Press 'r' to reset and draw new circles", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
